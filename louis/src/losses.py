...
class DistillLoss(PairwiseSimilarityLossBase):
    ...
    def forward(
        ...
        for m in self.m_list:
            ...
            sim_adapted = F.cosine_similarity(
                adapted_trunc[:, None, :],     # (N,1,m)
                adapted_trunc[topk_indices],   # (N,k,m)
                dim=-1
            )
            ...
            loss_m = diff.sum()
            total_loss += loss_m
            partial_losses[f'loss_{"q" if q else "p"}_topk_partial_{m}'] = loss_m.clone().detach()
            partial_losses[f'loss_{"q" if q else "p"}_topk_partial_{m}'] /= embeddings.shape[0]
            
            del adapted_trunc, sim_adapted, diff
