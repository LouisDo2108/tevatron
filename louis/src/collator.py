...
from utils import temporal_query_type_class_id, allen_relation_class_id
import torch
...
class TemporalReconCollator(TrainCollator):
    ...
    def __call__(self, features):
        ...
        qt_collated_list = self.tokenizer(
            ...
            [x for xs in all_query_temporal_list_list_str for x in (xs if xs != [] else [""])],
            padding=True,
            truncation=True,
            max_length=16,
            return_attention_mask=True,
        ...
        pt_collated_list = self.tokenizer(
            ...
            [x for xs in all_passages_temporal_list_list_str for x in (xs if xs != [] else [""])],
            padding=True,
            truncation=True,
            max_length=16,
            return_attention_mask=True,
