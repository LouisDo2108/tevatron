...
class LLMDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        docid = int(content["positive_passages"][0]["docid"][1:])
        query = normalize_and_split_string_by_punctuation(content["query"], add_title=True)
...
def llm_collate(batch):
    ...
    for docid, query_id, query_list, positive_passages in batch:
