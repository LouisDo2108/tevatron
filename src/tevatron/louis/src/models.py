...
@dataclass
class TempRetrieverEncoderOutput(ModelOutput):
...
class NaiveTemporal(DenseModel):
    def __init__(self, *args, **kwargs):
        ...
        if self.training_args:
            ...
            self.l2_reg = self.training_args.l2_reg
            self.cka_reg = self.training_args.cka_reg
...
class TemporalProjectorReconstruction(NaiveTemporal):
    def __init__(self, *args, **kwargs):
        ...
        self.distill_loss = DistillLoss(
            k=10, m_list=self.matryoshka_dim_list[1:]
        )
        self.cka_loss = LinearCKALoss(m_list=self.matryoshka_dim_list[1:])
        self.l2_reg_loss = nn.MSELoss()
    ...
    def _calc_l2_reg_loss(self, q_reps_base, p_reps_base, q_reps_trunc, p_reps_trunc):
        pass
    ...
    def compute_loss(
        ...
        if self.temporal:
            temporal_loss, temporal_partial_losses = self._calc_temporal_loss(
                q_reps, qt_reps, 
                p_reps, pt_reps, 
                qt_tokens_input_ids_list, pt_tokens_input_ids_list,
            )
            total_loss += temporal_loss
            partial_losses.update(temporal_partial_losses)

        if self.distillation:
            distillation_loss, distillation_partial_losses = self._calc_distillation_loss(q_reps, p_reps)
            total_loss += distillation_loss
            partial_losses.update(distillation_partial_losses)
        
        if self.l2_reg:
            pass
    
        if self.cka_reg:
            cka_reg_loss, cka_reg_partial_losses = self._calc_cka_reg_loss(q_reps, p_reps)
            total_loss += cka_reg_loss
            partial_losses.update(cka_reg_partial_losses)
        
        return total_loss, partial_losses

    def _calc_temporal_loss(self, q_reps, qt_reps, p_reps, pt_reps, qt_tokens_input_ids_list, pt_tokens_input_ids_list):
    ...
    def _calc_distillation_loss(self, q_reps, p_reps):
        total_loss = torch.tensor(0.0, device=q_reps.device)
        partial_losses = {}
        
        loss_topk_q, loss_topk_partial_q = self.distill_loss(
            self.q_reps_trunc, self.matryoshka_dim, q=True,
        )
            
        loss_topk_p, loss_topk_partial_p = self.distill_loss(
            self.p_reps_trunc, self.matryoshka_dim, q=False,
        )

        total_loss += self.distillation * (loss_topk_q + loss_topk_p)
        partial_losses.update(loss_topk_partial_q)
        partial_losses.update(loss_topk_partial_p)
        
        return total_loss, partial_losses

    def _calc_cka_reg_loss(self, q_reps, p_reps):
        total_loss = torch.tensor(0.0, device=q_reps.device)
        partial_losses = {}
        
        loss_cka_q, loss_cka_partial_q = self.cka_loss(
            self.q_reps_trunc, 
            target_dim=self.matryoshka_dim,
            q=True,
        )
        
        loss_cka_p, loss_cka_partial_p = self.cka_loss(
            self.p_reps_trunc, 
            target_dim=self.matryoshka_dim,
            q=False,
        )

        total_loss += self.cka_reg * (loss_cka_q + loss_cka_p)
        partial_losses.update(loss_cka_partial_q)
        partial_losses.update(loss_cka_partial_p)
...
class TempRetriever(NaiveTemporal):
    ...
    def forward(
        ...
        if self.training:
            ...
            # Copy from EncoderModel's forward
            q_reps, qt_reps = None, None
            p_reps, pt_reps = None, None
            if query is not None:
                q_reps, qt_reps = self.encode(query, qt) 
            if passage is not None:
                p_reps, pt_reps = self.encode(passage, pt) 

            if query is None or passage is None:
                return TempRetrieverEncoderOutput(q_reps=q_reps, qt_reps=qt_reps, p_reps=p_reps, pt_reps=pt_reps)
    ...
    def encode(self, q, qt=None):
        query_hidden_states = self.encoder(**q, return_dict=True)
        query_hidden_states = query_hidden_states.last_hidden_state
        pooled_hidden_states = self._pooling(query_hidden_states, q["attention_mask"])

        qt_pooled_hidden_states = None
        if qt != None:
    ...
    def compute_loss(
        ...
        q_reps = torch.mul(q_reps, qt_reps)
        p_reps = torch.mul(p_reps, pt_reps)
    ...
    def load(cls,
        ...
        # semantic_model.from_pretrained(model_name_or_path, weights_only=False, trust_remote_code=True,**hf_kwargs)
        
        # semantic_model.try_using_flash_attn(model_name_or_path, hf_kwargs)
        model = cls(
        ...
        logger.info(f"Loaded model from {model_name_or_path}/model.safetensors")
