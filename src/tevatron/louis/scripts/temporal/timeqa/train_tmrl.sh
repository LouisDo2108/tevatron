...
EXP_NAME="t_${TEMPORAL_DIM}_alpha_${QT}_only_t_and_768"

bash /home/thuy0050/code/TMRL/src/tevatron/louis/scripts/temporal/train.sh "$MODEL" "$DATA" "$EXP_NAME" "$BATCH_SIZE" "$QT" "$PT" "$DISTILLATION" "$CKA_REG" "$DETACH_TEMPORAL" "$TEMPORAL" "$FILTER_FALSE_NEGATIVES" "$GRADIENT_CHECKPOINTING" "$ADD_CLS" "$TEMPORAL_DIM" "$METHOD_NAME" "$ENHANCED_TEMPORAL"
...
# EXP_NAME="t_${TEMPORAL_DIM}_alpha_${QT}_no_filter_false_negatives"
...
# ADD_CLS=""
# GRADIENT_CHECKPOINTING=""
# EXP_NAME="t_${TEMPORAL_DIM}_alpha_${QT}_no_filter"
