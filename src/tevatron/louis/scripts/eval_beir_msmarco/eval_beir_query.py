...
def main():
    ...
    if METHOD_NAME == "zero-shot":    
        output_dir = OUTPUT_ROOT / "beir-msmarco-zero-shot" / backbone
    else:
    ...
    python {CODE_DIR}/src/tevatron/retriever/driver/{encode_file_name} \
        ...
        --encode_output_path {output_dir}/queries_emb_beir_msmarco.pkl \
    ...
    set -f && OMP_NUM_THREADS=16 python -m tevatron.retriever.driver.search \
        --query_reps {output_dir}/queries_emb_beir_msmarco.pkl \
        --passage_reps {output_dir}/corpus_emb_beir_msmarco.*.pkl \
        --depth 100 \
        --batch_size 2048 \
        --save_text \
        --save_ranking_to {output_dir}/rank_beir_msmarco.txt

    && python -m tevatron.utils.format.convert_result_to_trec \
        --input {output_dir}/rank_beir_msmarco.txt \
        --output {output_dir}/rank_beir_msmarco.trec \
        --remove_query
    
    && python -m tevatron.utils.format.convert_result_to_marco \
        --input {output_dir}/rank_beir_msmarco.txt \
        --output {output_dir}/rank_beir_msmarco.marco \

    && python -m pyserini.eval.msmarco_passage_eval \
        msmarco-passage-dev-subset \
        {output_dir}/rank_beir_msmarco.marco \
        > {output_dir}/out_beir_msmarco_marco.txt
    """
